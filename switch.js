...
export default class Switch extends DiscordBasePlugin {
    ...
    static get optionsSpecification() {
        return {
            ...
            showDisabledTimeRemaining: {
                required: false,
                description: "Show remaining time when the switch command is globally disabled by an admin.",
                default: true
            },
            maxDisableDurationMinutes: {
                required: false,
                description: "Maximum duration in minutes an admin can globally disable the switch command for.",
                default: 30
            },
    ...
    constructor(server, options, connectors) {
        ...
        // In-memory expiry for the admin global-disable feature (null = enabled)
    ...
    async safeTransaction(logicFn) {
        ...
        for (let i = 0; i < maxRetries; i++) {
            try {
                ...
                const isLocked = this.isTransactionLocked(err) || err?.name === 'SequelizeTimeoutError';
    ...
    async onChatMessage(info) {
        try {
            ...
            if (subCommand && subCommand != '') {
                ...
                case 'disable':
                    ...
                    this.verbose(1, `[Admin] Command '${subCommand}' accepted from ${playerName}`);
                    {
                        ...
                        this.warn(steamID, `Command temporarily disabled for ${minutes} minute(s).`);
                ...
                case 'double':
                    ...
                    pl = this.getPlayerByUsernameOrSteamID(steamID, commandSplit.splice(1).join(' '))
                    if (pl) {
                        await this.doubleSwitchPlayer(pl.steamID, true, steamID);
                    }
                    break;
                case 'squad':
                    if (!isAdmin) {
                ...
                case "doublesquad":
                    ...
                    await this.server.updateSquadList();
                    await this.server.updatePlayerList();
                    await this.doubleSwitchSquad(+commandSplit[ 1 ], commandSplit[ 2 ], steamID);
                    break;
                case "matchendsquad":
                    if (!isAdmin) {
                        this.verbose(1, `[Denied] Player ${playerName} (not admin) attempted admin command: ${subCommand}`);
            ...
            // Global disable (admin) — block non-admins before any queue/eligibility logic
            if (!isAdmin && this.globalDisabledExpiry && new Date() < this.globalDisabledExpiry) {
                let disabledMsg = `Command temporarily disabled.`;
    ...
    async switchSquad(number, team, steamIDCmdExecutor) {
    ...
    getPlayersFromSquad(number, team, steamIDCmdExecutor) {
        ...
        if (!team_id) {
            if (steamIDCmdExecutor) this.warn(steamIDCmdExecutor, `Could not find a faction from: "${team}".`);
    ...
    async addSquadToMatchendSwitches(number, team, steamIDCmdExecutor) {
        ...
        if (steamIDCmdExecutor) this.warn(steamIDCmdExecutor, `Added Team ${team} Squad ${number} to match end queue\n(${players.length} players)\n\n"!switch list" -> view queued players\n\n"!switch wipequeue" -> to clear.`);
    }
